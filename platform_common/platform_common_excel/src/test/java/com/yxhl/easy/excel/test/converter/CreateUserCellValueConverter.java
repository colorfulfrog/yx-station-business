package com.yxhl.easy.excel.test.converter;
...
public class CreateUserCellValueConverter implements CellValueConverter{
	...
	public Object convert(Object bean, Object value, FieldValue fieldValue, Type type, int rowNum)
			throws Exception {
		System.out.println("com.yxhl.easy.excel.test.converter.CreateUserCellValueConverter:20行输出："+fieldValue.getOtherConfig());
		//如果是导入
		if(type==Type.IMPORT){
	...
	private boolean queryForDb(String createUser){
		if(createUser.startsWith("王五")){
			System.out.println("数据库有查询到王五,发生在com.yxhl.easy.excel.test.converter.CreateUserCellValueConverter");
			return true;
		}
		return false;
	}
