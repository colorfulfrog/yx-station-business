 ...
 */
package com.elead.platform.common.utils.excel;
...
import com.elead.platform.common.utils.Reflections;
import com.elead.platform.common.utils.excel.annotation.ExcelField;
import com.google.common.collect.Lists;
//import com.elead.platform.common.utils.DictUtils;
...
public class ImportExcel {
	...
	private static SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");// 格式化日期，和数据库中日期格式相同，这里是datetime
	/**
	 * 工作薄对象
	 */
	private Workbook wb;
	...
	public ImportExcel(String fileName, InputStream is, int headerNum, int sheetIndex) throws InvalidFormatException, IOException {
		if (StringUtils.isBlank(fileName)) {
			...
			this.evaluator = new HSSFFormulaEvaluator((HSSFWorkbook) this.wb);

		} else if (fileName.toLowerCase().endsWith("xlsx")) {
			this.wb = new XSSFWorkbook(is);
			this.evaluator = new XSSFFormulaEvaluator((XSSFWorkbook) this.wb);
		} else {
		...
		try {
			...
			if (cell != null) {
				if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
					...
					} else if (cell.getCellStyle().getDataFormat() == 14 || cell.getCellStyle().getDataFormat() == 31 || cell.getCellStyle().getDataFormat() == 57) {
						// 如果是excel自定义日期格式，获取该cell的date值
						val = cell.getNumericCellValue();
						val = DateUtil.getJavaDate((Double) val);
					} else { // 纯数字
						double cellValue = cell.getNumericCellValue();
						val = new DecimalFormat("##.###").format(cellValue);
					}
					...
					try {
						CellValue cellValue;
						cellValue = this.evaluator.evaluate(cell);
						switch (cellValue.getCellType()) { // 判断公式类型
						case Cell.CELL_TYPE_BOOLEAN:
							val = String.valueOf(cellValue.getBooleanValue());
							break;
						case Cell.CELL_TYPE_NUMERIC:
							if (HSSFDateUtil.isCellDateFormatted(cell)) {
								...
								val = new DecimalFormat("#").format(dc);
							}
...
//							val = DateUtil.getJavaDate((Double) val);
	...
	public static void main(String[] args) throws Throwable {
		...
		ImportExcel ei = new ImportExcel("D:\\ac.xlsx", 0, 0);
